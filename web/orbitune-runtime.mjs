...
export function parseSafetensors(arrayBuffer) {
  ...
  for (const [name, spec] of Object.entries(header)) {
    ...
    const [relativeStart, relativeEnd] = spec.data_offsets;
    const bytes = relativeEnd - relativeStart;
...
function copyInto(destination, destinationOffset, tensor, expectedShape, name) {
  if (JSON.stringify(tensor.shape) !== JSON.stringify(expectedShape)) {
    throw new Error(`${name} shape ${JSON.stringify(tensor.shape)} != ${JSON.stringify(expectedShape)}`);
  }
...
export function emptyAdapterInputs() {
  return {
    loraA: new Float32Array(ORBITUNE_V0.layers * 2 * ORBITUNE_V0.rank * ORBITUNE_V0.hidden),
    loraB: new Float32Array(ORBITUNE_V0.layers * 2 * ORBITUNE_V0.hidden * ORBITUNE_V0.rank),
...
export function packAdapterSafetensors(arrayBuffer) {
  ...
  if (rank !== ORBITUNE_V0.rank) throw new Error(`Orbitune v0 browser runtime requires rank ${ORBITUNE_V0.rank}`);
  ...
  for (let layer = 0; layer < ORBITUNE_V0.layers; layer += 1) {
    for (let targetIndex = 0; targetIndex < 2; targetIndex += 1) {
      ...
      const prefix = `blocks.${layer}.attn.${target}`;
      const tensorA = tensors.get(`${prefix}.lora_a`);
      const tensorB = tensors.get(`${prefix}.lora_b`);
      if (!tensorA || !tensorB) throw new Error(`Adapter is missing ${prefix} LoRA tensors`);
      const aOffset = (layer * 2 + targetIndex) * ORBITUNE_V0.rank * ORBITUNE_V0.hidden;
      const bOffset = (layer * 2 + targetIndex) * ORBITUNE_V0.hidden * ORBITUNE_V0.rank;
      copyInto(packed.loraA, aOffset, tensorA, [ORBITUNE_V0.rank, ORBITUNE_V0.hidden], `${prefix}.lora_a`);
      copyInto(packed.loraB, bOffset, tensorB, [ORBITUNE_V0.hidden, ORBITUNE_V0.rank], `${prefix}.lora_b`);
    }
  }
  packed.scale[0] = alpha / rank;
  return packed;
}
...
export function tokenIdsToEvents(tokenIds) {
  ...
  while (i < tokenIds.length) {
    const token = VOCAB[tokenIds[i]];
    if (token === 'BAR') { bar += 1; i += 1; continue; }
    if (!token?.startsWith('POSITION_') || i + 3 >= tokenIds.length) { i += 1; continue; }
    const position = Number(token.slice('POSITION_'.length));
    const pitchToken = VOCAB[tokenIds[i + 1]];
    const durationToken = VOCAB[tokenIds[i + 2]];
    const velocityToken = VOCAB[tokenIds[i + 3]];
    ...
    events.push({
      bar: Math.max(0, bar),
      position,
      pitch: Number(pitchToken.slice('NOTE_PITCH_'.length)),
      duration: Number(durationToken.slice('NOTE_DURATION_'.length)),
      velocity: Math.max(1, Math.min(127, Math.round(Number(velocityToken.slice('VELOCITY_'.length)) / 32 * 127))),
    });
    i += 4;
  }
  return events;
...
export function eventsToMidiBytes(events, bpm = 84) {
  ...
  const ticksPerPosition = 120;
  const scheduled = [];
  const tempo = Math.round(60000000 / bpm);
  scheduled.push({ time: 0, data: [0xff, 0x51, 0x03, (tempo >>> 16) & 0xff, (tempo >>> 8) & 0xff, tempo & 0xff] });
  for (const event of events) {
    const start = (event.bar * 16 + event.position) * ticksPerPosition;
    const end = start + Math.max(1, event.duration) * ticksPerPosition;
    scheduled.push({ time: start, data: [0x90, event.pitch, event.velocity] });
    scheduled.push({ time: end, data: [0x80, event.pitch, 0] });
  }
...
export class OrbituneBrowserRuntime {
  constructor(ortNamespace) {
    if (!ortNamespace) throw new Error('ONNX Runtime Web namespace is required');
  ...
  async logitsFor(tokenIds) {
    ...
    const inputIds = new BigInt64Array(context.map((id) => BigInt(id)));
    const feeds = {
      input_ids: new this.ort.Tensor('int64', inputIds, [1, context.length]),
      lora_a: new this.ort.Tensor('float32', this.adapter.loraA, [4, 2, 4, 240]),
      lora_b: new this.ort.Tensor('float32', this.adapter.loraB, [4, 2, 240, 4]),
      lora_scale: new this.ort.Tensor('float32', this.adapter.scale, [1]),
    };
  ...
  async generate({ bars = 8, temperature = 0.85, topP = 0.92, maxNewTokens = 2048 } = {}) {
    ...
    for (let step = 0; step < maxNewTokens; step += 1) {
      const allowed = allowedNextTokenIds(ids, bars);
      if (!allowed.length) break;
      const logits = await this.logitsFor(ids);
      const next = sampleAllowedLogits(logits, allowed, { temperature, topP });
      ids.push(next);
      if (VOCAB[next] === 'EOS') break;
    }
