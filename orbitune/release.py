...
def package_base_release(base: str | Path, web_onnx: str | Path, out_dir: str | Path, *, repository: str = "Unjuno/orbitune", release_tag: str = BASE_MODEL_ID) -> dict[str, object]:
    ...
    shutil.copy2(base, checkpoint_out); shutil.copy2(web_onnx, onnx_out)
    ...
    manifest: dict[str, object] = {
        "schema_version": "0.1.0", "model_id": BASE_MODEL_ID,
        "immutability": "published checkpoint bytes must never be replaced",
        "architecture": ARCHITECTURE_ABI, "tokenizer": TOKENIZER_ABI,
        "parameters": BASE_PARAMETER_COUNT, "base_sha256": checkpoint_record["sha256"],
        "config": asdict(model.config), "release": {"repository": repository, "tag": release_tag},
    ...
    runtime_config = {"model_url": onnx_record["url"], "model_sha256": onnx_record["sha256"], "base_sha256": checkpoint_record["sha256"], "execution_providers": ["wasm"]}
    (out_dir / "runtime-config.json").write_text(json.dumps(runtime_config, indent=2) + "\n", encoding="utf-8")
    return manifest

# CI-only marker for multi-Base registry verification.
