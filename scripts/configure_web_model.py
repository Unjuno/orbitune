...
def validate_runtime_config(config: dict[str, Any], *, allow_unpublished: bool = False) -> None:
    ...
    if not str(model_url).startswith("https://"):
...
def main() -> int:
    ...
    parser.add_argument("runtime_config", help="generated runtime-config.json from package_base_release.py")
    parser.add_argument("--out", default="web/runtime-config.json")
