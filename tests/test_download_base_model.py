...
def _manifest(payload: bytes) -> dict[str, object]:
    ...
    return {
        ...
        "artifacts": {
            "checkpoint": {
                "filename": "orbitune-base.pt",
                "bytes": len(payload),
                "sha256": digest,
                "url": "https://example.test/orbitune-base.pt",
            }
        },
    }
...
def test_validate_artifact_accepts_immutable_base_manifest():
    artifact = downloader._validate_artifact(_manifest(b"checkpoint"), "checkpoint")
    assert artifact["filename"] == "orbitune-base.pt"
...
def test_download_verified_checks_hash_before_replace(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(downloader.urllib.request, "urlopen", lambda *args, **kwargs: io.BytesIO(payload))
    target = tmp_path / "orbitune-base.pt"
    downloader._download_verified(artifact, target)
    assert target.read_bytes() == payload
    assert downloader.sha256_file(target) == artifact["sha256"]
...
def test_download_verified_removes_corrupt_partial_file(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(downloader.urllib.request, "urlopen", lambda *args, **kwargs: io.BytesIO(b"corrupt"))
    target = tmp_path / "orbitune-base.pt"
